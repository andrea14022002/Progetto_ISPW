...
import com.plantnursery.view.cli.NotificationsView;
...
public class NotificationsGUIControllerCLI extends AbstractGUIControllerCLI {
    ...
    public void start(){
        ...
        try {
            ...
            notifications = notificationsController.getNotifications((SellerBean) user);
    ...
    private void showNotifs() {
        ...
        for (int i = 0; i < notifications.size(); i++) {
            ...
            notifStrings[i] = String.format("%d - Type: %s, Event: %s, Booking Code: %s, DateTime: %s - %s%n", i+1, notifications.get(i).getType(),
                    notifications.get(i).getSetName(), notifications.get(i).getOrderCode(), date, time);
        }
    ...
    private void deleteNotifs() {
        try {
            ...
            if (numDelete.contains(-1)) {
                notificationsController.deleteAllNotifications((SellerBean) SessionManager.getSessionManager().getSessionFromId(currentSession).getUser());
                notifications.clear();
            } else {
                ...
                notificationsController.deleteNotifications(notifsToDelete, (SellerBean) user);
